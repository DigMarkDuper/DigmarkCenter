...
# --- HALAMAN 5: DATABASE NOMOR (CRM) ---
        ...
        # ID SPREADSHEET (Sudah sesuai URL Mas)
        ...
        with c_sync:
            ...
            if st.button("Tarik Data Unik dari WA Admin", use_container_width=True, key="sync_crm_v5"):
                sync_leads_to_crm() 
        ...
        with c_upload:
            ...
            with st.expander("Upload File Excel (.xlsx)"):
                ...
                st.download_button(label="📥 Download Template", data=buffer_template.getvalue(), file_name="Template_CRM.xlsx", use_container_width=True, key="dl_v5")
                
                # --- FITUR UPLOAD ---
                uploaded_file = st.file_uploader("Upload file Excel", type=["xlsx"], key="up_v5")
                if uploaded_file is not None:
                    try:
                        df_upload = pd.read_excel(uploaded_file)
                        df_upload.columns = [str(c).strip().lower() for c in df_upload.columns]
                        
                        if st.button("📥 Konfirmasi Import Massal", use_container_width=True, key="conf_v5"):
                            with st.spinner("Mengirim data..."):
                                tgl_hari_ini = datetime.date.today().strftime("%d-%m-%Y")
                                df_upload = df_upload.fillna("")
                                
                                # --- PENYESUAIAN URUTAN KOLOM ---
                                # A: No, B: Nama, C: No Hp, D: Domisili, E: Kosong (Lahir), F: Tgl Masuk
                                bulk_data = []
                                ...
                                # Eksekusi ke tab ke-5 (indeks 4)
                                if append_sheet_rows(4, bulk_data):
                                    st.success(f"🚀 Berhasil! Data masuk di kolom Tanggal Masuk Database.")
                                    st.cache_data.clear()
                                    st.rerun()
                    except Exception as e:
                        st.error(f"Gagal: {e}")
        ...
        try:
            ...
            if not df_crm.empty:
                ...
                with st.expander("🔍 Filter Strategis Database", expanded=True):
                    search_crm = st.text_input("🔎 Cari Nama/HP:", placeholder="Ketik...", key="search_v5")
                    c1, c2 = st.columns(2)
                    with c1:
                        # Ambil kolom mekari tag jika ada
                        m_col = 'Mekari Tag (Status Terakhir)'
                        opts_mekari = sorted(df_crm[m_col].unique().tolist()) if m_col in df_crm.columns else []
                        sel_mekari = st.multiselect("Mekari Tag:", options=opts_mekari, key="f_mek_v5")
                    with c2:
                        opts_daerah = sorted(df_crm['Domisili'].unique().tolist()) if 'Domisili' in df_crm.columns else []
                        sel_daerah = st.multiselect("Pilih Daerah:", options=opts_daerah, key="f_daer_v5")
                ...
                if sel_mekari:
                    mask &= df_crm[m_col].isin(sel_mekari)
                if sel_daerah:

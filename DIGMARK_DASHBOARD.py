        ...
        if up_mk is not None:
            try:
                ...
                # 2. Deteksi File: CONVERSATION / BILLING LOGS (Chat Individu)
                    ...
                    # FIX: Jangan SUM kolom credit. Ambil selisih Saldo Awal dan Saldo Akhir.
                    series_credit = pd.to_numeric(df_calc_mk['credit'], errors='coerce').dropna()
                    if not series_credit.empty:
                        # Saldo terpakai = Nilai awal - Nilai akhir (asumsi urutan waktu)
                        up_spend_mk = float(abs(series_credit.iloc[0] - series_credit.iloc[-1]))
                    else:
                        up_spend_mk = 0.0
                ...
                st.info(f"📅 **Periode Data:** {periode_data}\n\n📊 **Total Saldo Terpakai:** Rp {up_spend_mk:,.0f} | **Jumlah Pesan:** {up_pesan_mk:,.0f}")
